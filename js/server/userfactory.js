...
/*globals storage, logging, Enumerable*/
/*exported userFactory, md5*/
"use strict";
...
var userFactory = (function ($, storage, logging) {
	...
	return {
		...
		getOnValidSignIn: function (email, password) {
			var hash = md5(password);
			return Enumerable.From(loadRaw())
				.Where(function (x) { return x.email === email && x.passwordHash === hash; })
				.Select(userMap)
				.FirstOrDefault();
		}
	};
